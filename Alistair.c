...
int TrailingStop() {
	...
	// adjust the stop only when the trade is in profit  if(TradeProfit > 0)// place the stop at the lowest bottom of the previous 3 candles
...
function BuyAlgo(var EMA_H, EMA_L, TimePeriod, tr)	// Buy signal
{
...
#include "Strategy\include\plot.h"
