...
// #include "Strategy\include\objective.h"
	bool b1,b2,b3,b4,s1,s2,s3,s4	=	false;				//трябва да стои извън if(is(INITRUN))
...
function BuyAlgo(var EMA_H, EMA_L, TimePeriod, tp, tr)	// Buy signal
{
	...
	if (!b1 and (price() < LL(TimePeriod,1)))	{
		b1 = true;														//пресичане на LL
		return;
	}
	if (price() > HH(TimePeriod,1))	{
		b1 = false;														//прекратяване на сигнала за buy заради пресичане HH
		return;
	}
	...
	if (b2 and (priceOpen() > EMA_H))	{
		b3 = true;														//затворен бар над на MA5(High)
		return;
	}
...
function SellAlgo(var EMA_H, EMA_L, TimePeriod, tp, tr)	// Sell signal
{
...
function run() 
{
	...
	var TimePeriod	=	100; //optimize (1000,100,1400,100);
