...
bool b1,b2,b3,b4,s1,s2,s3,s4	=	false;	//глобални, трябва да стоят извън if(is(INITRUN))
vars sRSI = series(RSI(series(price()),14));
...
int TrailingStop() { // TMF that adjusts the stop in a special way 
	...
	if (crossUnder(sRSI,70) or crossOver(sRSI,30))
		return 1;

}
function BuyAlgo(var EMA_H, EMA_L, TimePeriod)	// Buy signal
{
...
function SellAlgo(var EMA_H, EMA_L, TimePeriod)	// Buy signal
{
...
function run() 
{
	...
	BarPeriod 		=	15;
	LookBack 		=	1000; 
	var TimePeriod	=	800; //optimize (1000,100,1400,100);
	...
	while(asset(loop(Assets))) {
		while(algo(loop("BUY","SELL")))
			switch (Algo) {
				// case	"BUY"	: BuyAlgo(EMA_H, EMA_L, TimePeriod);
				case 	"SELL"	: SellAlgo(EMA_H, EMA_L, TimePeriod);
			}
	}
